...
![](https://images-cdn.shimo.im/st4hx99y114OPNwf/Screen_Shot_2018_05_15_at_5.55.42_PM.png!thumbnail)
### 虚拟链路层的设计
...
### RRIP协议的实现TODO:
RIP协议主要在RIP类中实现。每个运行RIP协议的路由器需要维护以下数据结构：
``` python
...
```
RIP协议在网络层初始化完毕后启动。RIP协议启动时（即RIP类构造时），只需要知道本地链路的信息。并用本地链路信息初始化以上条目。

route_name和interfaces由网路层传入，RIP类把保存下来。
...
RIP.process函数是RIP协议的重点，在收到rip_msg后它的处理过程共分五步：
检查RIP报文的所有项，如果该项和已知的离线状态的路由器有关，则抛弃该报文。
检查rip_msg中邻居的tear_down列表，确认是否又有其他路由器离线了，如果有，则删除自身维护的数据结构中与该离线路由器相关的项。

检查rip_msg中邻居的距离向量，如果有其中某些键（route_name）不在自身的距离向量的键中，则说明发现了新的与自身不直接相连的路由器，更新自己的距离向量和拓扑图。

用rip_msg中新的邻居距离向量更新自身在距离表中储存的旧值。
根据更新过的距离向量和距离表，计算新的到所有已知路由器的最短路径并更新路由表。
...
#### test2文件夹
![](https://images-cdn.shimo.im/1mjJaOVSePUZEkmP/image.png!thumbnail)

### 测试项目一：虚拟路由器的静态路由配置
在实现路由选择协议前，我们实现了虚拟的链路层及网络层，在这些层提供的接口之上，我们实现了一个能够对路由器各项属性进行查看及配置的控制台，以下视频是我们的测试结果。我们的测试视频存放在TODO:路径？
...
可见视频：TODO:路径ok
（not implemented)场景3：【链路费用改变】此时路由器可能会发现一条更优的路径，修改存储拓扑结构的数据库和转发表，并广播此改变信息。
可见视频:  TODO:路径
### 测试项目四：中心化路由
