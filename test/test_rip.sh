...
# 给路由器A发送初始化脚本
tmux send -t "init":0.0 "debug start;start rip;sleep 10;debug stop$COMMAND_FOR_ALL" Enter

# 给路由器B发送初始化脚本
tmux send -t "init":0.1 "debug start;start rip;sleep 10;debug stop$COMMAND_FOR_ALL" Enter

# 给路由器C发送初始化脚本
tmux send -t "init":0.2 "debug start;start rip;sleep 10;debug stop$COMMAND_FOR_ALL" Enter

# 给路由器D发送初始化脚本
tmux send -t "init":0.3 "debug start;start rip;sleep 10;debug stop$COMMAND_FOR_ALL" Enter

# 给路由器E发送初始化脚本
tmux send -t "init":0.4 "debug start;start rip;sleep 10;debug stop$COMMAND_FOR_ALL" Enter
